...
class LanguageModel:
    ...
    def sample(self, max_count: int) -> str: 
        """Generate a sentence from BOS to EOS"""
        x, y = BOS, BOS
        z: Wordtype = ""
        sentence = ""
        count = 0
        ...
        while count < max_count:
            ...
            for word in self.vocab:
                probs.append(math.exp(self.log_prob(x, y, word)))
                words.append(word)
            count += 1
            z = random.choices(words, probs)[0]
            if z == EOS:
                break
            sentence += z + " "
            x = y
            y = z
        if count == max_count and z!= EOS:
            sentence += "..."
        else:
            sentence += "."
        return sentence
...
class BackoffAddLambdaLanguageModel(AddLambdaLanguageModel):
    ...
    def prob(self, x: Wordtype, y: Wordtype, z: Wordtype) -> float:
        ...
        zprob = (self.event_count[z,]  + (self.lambda_)) / (self.context_count[()] + (self.lambda_ * self.vocab_size))
        yzprob = (self.event_count[y,z]  + (self.lambda_ * self.vocab_size * zprob)) / (self.context_count[y,] + (self.lambda_ * self.vocab_size))
        xyzprob = (self.event_count[x,y,z] + (self.lambda_ * self.vocab_size * yzprob)) / (self.context_count[x,y] + (self.lambda_ * self.vocab_size))
        return xyzprob
...
class EmbeddingLogLinearLanguageModel(LanguageModel, nn.Module):
    ...
    def __init__(self, vocab: Vocab, lexicon_file: Path, l2: float, epochs: int) -> None:
        ...
        data_matrix = []
        self.word_index_map: Integerizer[Wordtype]
        self.word_index_map = Integerizer([])
        # load lexicon from file 
        with open(lexicon_file) as f:
            first_line = next(f)  # Peel off the special first line.
            self.dim = int(first_line.split()[1])
            for line in f:  # All of the other lines are regular.
                line_tokens = line.split("\t")
                if line_tokens[0] in vocab or line_tokens[0] == OOL:
                    self.word_index_map.add(line_tokens[0]) # word 
                    vector: Vector = [float(x) for x in line_tokens[1:]]
                    data_matrix.append(vector) # numbers
        # self.word_embbedding = torch.tensor(data_matrix, requires_grad=True)
        self.E = torch.tensor(data_matrix)
    ...
    def prob(self, x: Wordtype, y: Wordtype, z: Wordtype) -> float:
        """Return log p(z | xy) according to this language model."""
        # https://pytorch.org/docs/stable/generated/torch.Tensor.item.html
        return math.exp(self.log_prob(x, y, z))
    ...
    def log_prob_tensor(self, x: Wordtype, y: Wordtype, z: Wordtype) -> TorchScalar:
        ...
        logits_vector = self.logits(x, y)

        z_vector = self.get_embedding(z)
        return (self.get_embedding(x) @ self.X @ z_vector.t() + (self.get_embedding(y) @ self.Y @ z_vector.t()) ) - torch.logsumexp(logits_vector,0)

        #minseo's versio: 
        # return torch.log_softmax(torch.logsumexp(logits_vector), dim=self.word_index_map.index(z))
    def get_embedding(self, x) -> Float[torch.Tensor]:
        if x != OOV and x in self.word_index_map: 
            return (self.E[self.word_index_map.index(x)])
        else:
            return (self.E[self.word_index_map.index(OOL)])
    ...
    def logits(self, x: Wordtype, y: Wordtype) -> Float[torch.Tensor,"vocab"]:
        ...
        # x, y, and z are words in the files. 
        x_vector = self.get_embedding(x)
        y_vector = self.get_embedding(y)
        return x_vector @ self.X @ self.E.t() + y_vector @ self.Y @ self.E.t()
    ...
    def train(self, file: Path):    # type: ignore
        ...
        for a in range(self.epochs):
            ...
            for x,y,z in tqdm.tqdm(read_trigrams(file, self.vocab), total=N):
                # print(x+ y+z)
                log_prob: TorchScalar = self.log_prob_tensor(x, y, z)
                regularizer: TorchScalar = (self.l2 / N) * (torch.norm(self.X, "fro")**2 + torch.norm(self.Y, "fro")**2)
                F_i= log_prob - regularizer
                (-F_i).backward()
                F += F_i
                optimizer.step()
                optimizer.zero_grad()
            print("epoch " + str(a+1) + ": F = " + str((F/N).item()))
            # break
...
class ImprovedLogLinearLanguageModel(EmbeddingLogLinearLanguageModel):
    ...
    def __init__(self, vocab: Vocab, lexicon_file: Path, l2: float, epochs: int) -> None:
        ...
        self.ool = nn.Parameter(torch.tensor([0.15]), requires_grad=True)
    ...
    def log_prob_tensor(self, x: Wordtype, y: Wordtype, z: Wordtype) -> TorchScalar:
        ...
        # will be very slow. Some useful functions of pytorch that could
        # be useful are torch.logsumexp and torch.log_softmax.
        logits_vector = self.logits(x,y,z)
        x_vector = self.get_embedding(x)
        y_vector = self.get_embedding(y)
        z_vector = self.get_embedding(z)
        logits_vector[self.word_index_map.index(OOL)] *= self.ool.item()
        return ((x_vector @ self.X @ z_vector.t()) + (y_vector @ self.Y @ z_vector.t()) ) - torch.logsumexp(logits_vector,0)

    def get_embedding(self, x) -> Float[torch.Tensor]:
        if x != OOV and x in self.word_index_map: 
            return (self.E[self.word_index_map.index(x)])
        else:
            return (self.E[self.word_index_map.index(OOL)]) * self.ool
        

    def logits(self, x: Wordtype, y: Wordtype, z: Wordtype) -> Float[torch.Tensor,"vocab"]:
        """Return a vector of the logs of the unnormalized probabilities f(xyz) * θ 
        for the various types z in the vocabulary.
        These are commonly known as "logits" or "log-odds": the values that you 
        exponentiate and renormalize in order to get a probability distribution."""
